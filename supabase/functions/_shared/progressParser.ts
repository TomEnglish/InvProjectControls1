// IMPORTANT: this file is a near-verbatim copy of frontend/src/lib/progressParser.ts.
// The only intentional divergence is the xlsx import line:
//   - frontend uses Vite's bundler resolution: `import * as XLSX from 'xlsx'`
//   - edge-fn Deno uses npm specifier: `import * as XLSX from 'npm:xlsx@0.18.5'`
// Pin the xlsx version to match frontend/package.json. A parity test
// (frontend/src/lib/progressParser.parity.test.ts) verifies the two files
// match line-for-line (after stripping the import line + this header) so
// accidental drift fails CI.
//
// Used by queue-progress-upload (clerk submission flow — parses the file
// server-side before queueing). queue-approve-upload reads cached
// parsed.json from Storage, NOT this parser, so a hypothetical parser
// drift between submit and approve doesn't affect committed records —
// the parsed_rows blob at submit time is what gets imported.
